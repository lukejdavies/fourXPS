obs.Sim.4MOST<-function(simName=simName,id=id, EXPSec=EXPSec, NSub=NSub, airMass=1.4, IQ=1.1, skyBright=21.77,tilt=6.0, misAlign=0.1, writeFITSOut=FALSE, ETCPath='/Users/lukehome/work/ICRAR_work/4MOST/IWG8/Mock_test/4FS-ETC_app/4most2/code/bin/', systemModelDir='/Users/lukehome/work/ICRAR_work/4MOST/IWG8/Mock_test/4FS-ETC_app/4FS_ETC_system_model_v0.2/', verbose=F){
  ...
  out<-list(Sim=simName,NSpec=length(id))
  
  for (i in 1:length(id)){
    
    if (verbose==T){cat('Running Source ',i,' of ',length(id), '....', '\n')}
    
    if (length(EXPSec)==1){EXPSec2<-EXPSec}else{EXPSec2<-EXPSec[i]}
    if (length(NSub)==1){NSub2<-NSub}else{NSub2<-NSub[i]}
    if (length(airMass)==1){airMass2<-airMass}else{airMass2<-airMass[i]}
    if (length(IQ)==1){IQ2<-IQ}else{IQ2<-IQ[i]}
    if (length(skyBright)==1){skyBright2<-skyBright}else{skyBright2<-skyBright[i]}
    if (length(tilt)==1){tilt2<-tilt}else{tilt2<-tilt[i]}
    if (length(misAlign)==1){misAlign2<-misAlign}else{misAlign2<-misAlign[i]}
    
    OutName<-paste('4MOSTObs_',simName,'/',simName,'_',id[i],'.fits',sep='')
  
    spec<-readFITS(paste(simName,'/',simName,'_',id[i],'.fits', sep=''))
    FluxOrig<-spec$imDat
    ...
    make.ETCpar(simName,id[i], EXPSec2, NSub2, specMag, specMag, airMass=airMass2, IQ=IQ2, skyBright=skyBright2,tilt=tilt2, misAlign=misAlign2, systemModelDir=systemModelDir)
    ...
    conditions<-readFITS(paste('tmpETC',simName,'/specout_template_',simName,'_',id[i],'_LRS_red.fits',sep=''), hdu=1)
    results_r<-readFITS(paste('tmpETC',simName,'/specout_template_',simName,'_',id[i],'_LRS_red.fits',sep=''), hdu=2)
    results_b<-readFITS(paste('tmpETC',simName,'/specout_template_',simName,'_',id[i],'_LRS_blue.fits',sep=''), hdu=2)
    results_g<-readFITS(paste('tmpETC',simName,'/specout_template_',simName,'_',id[i],'_LRS_green.fits',sep=''), hdu=2)
    ...
    FluxOrig2<-approx(WaveOrig,FluxOrig,wave)$y
      ...
      if (writeFITSOut==TRUE){
        ...
        write.fitskey('OBJECT',id[i], OutName, comment = 'Input id', hdu = 1)    
        ...
        write.fitskey('EXP',EXPSec2, OutName, comment = 'Exposure Time, Sec', hdu = 1)
        write.fitskey('NSUB',NSub2, OutName, comment = 'Number of Sub Exposures', hdu = 1)
        write.fitskey('AIRMASS',airMass2, OutName, comment = 'Simulated AirMass', hdu = 1)
        write.fitskey('IQ',IQ2, OutName, comment = 'Simulated Image Quality', hdu = 1)
        write.fitskey('SKYBRI',skyBright2, OutName, comment = 'Simulated Sky Brightness', hdu = 1)
        write.fitskey('TILT',tilt2, OutName, comment = 'Simulated Fibre Tilt', hdu = 1)
        write.fitskey('MISALIG',misAlign2, OutName, comment = 'Simulated Fibre Misalignment', hdu = 1)
      }
        
      tmp<-list(id=id[i], wave=wave, flux=flux,error=error, EXPSec=EXPSec2, Nsub=NSub2, airMass=airMass2, IQ=IQ2, skyBright=skyBright2, tilt=tilt2, misAlign=misAlign2,FluxOrig=FluxOrig2,z=specZ,band=band,mag=specMag, xunit='ang',yunit='ang', axDat=axDat)
      name<-paste('spec',i,sep='')
      out[[name]] <- tmp
